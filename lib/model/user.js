...
exports.getCollars = function(userId, cb)
{
	...
	eventDao.getCollars(userId, ["ConnectionUserId", "ConnectionCount"], function (err, collars){
		...
		for (var i = 0 ; i < collars.length; i++)
		{
			...
			console.log('items : ' + JSON.stringify(collars[i]));
			functions.push(function (callback){
				eventDao.getUser(id, ["UserId", "ImageUrl"], function (err, users){
					users.ConnectionCount = count;
					callback(err, users);
				});
			});
		}
...
exports.getProfile = function(userId, connectionUserId, cb)
{
	...
	eventDao = new EventDao();
	eventDao.getUser(userId, attributes, function (err, users){
		console.log(users);
		
		eventDao.getImages(userId, null, function (err, images){
			if (users != null)
			{
				users.Images = images; 	
			}
			eventDao.getCollar(userId, connectionUserId, function (err, collar){
				...
				for (var i = 0 ; i < connections.length; i++)
				{
					...
					functions.push(function (callback){
						eventDao.getConnection(userId, id, function (err, connections){
